...
export const userTrainDataRoutes = async (app: FastifyInstance) => {
  app.withTypeProvider<ZodTypeProvider>().route({
    method: "GET",
    url: "/",
    schema: {
      tags: ["user-train-data"],
    ...
    handler: async (request, reply) => {
      try {
        ...
        const result = await getTrainData.execute({
  ...
  app.withTypeProvider<ZodTypeProvider>().route({
    ...
    schema: {
      tags: ["user-train-data"],
      summary: "Update current user training data",
